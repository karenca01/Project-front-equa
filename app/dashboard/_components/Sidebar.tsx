...
export default function Sidebar(){
    const handleEventSearch = async (id: string) => {
    ...
    return(
        <nav className="w-3/12 h-[90vh] bg-gris-claro flex flex-col items-center py-20 justify-center gap-10">
            ...
            <div className="flex flex-row justify-center items-center gap-4">
                <NavItem icon={<LuBuilding className="text-4xl w-30 border-1"/>} path="/dashboard"/>
                <NavItem icon={<LuCalendar className="text-4xl w-30 border-1"/>} path="/dashboard/users/"/>
            </div>
            <div className="flex flex-row justify-center items-center gap-4">
                <NavItem icon={<LuCalendar className="text-4xl w-30 border-1"/>} path="/dashboard/users/"/>
                <NavItem icon={<LuBuilding className="text-4xl w-30 border-1"/>} path="/dashboard"/>
